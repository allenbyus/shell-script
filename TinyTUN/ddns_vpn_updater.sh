...
doUpdate(){
	...
	ip route add 59.120.227.144/32 via 156.96.105.1 dev eth0
	ip route add ${1} via 156.96.105.1 dev eth0
	ip route del ${2} via 156.96.105.1 dev eth0
...
if [[ "${network_check}" == "000" ]];then
	...
	pkill tinytun
	ip route del default
	ip route add default via 156.96.105.1 dev eth0
else
	echo "网络连接：OK"
fi
